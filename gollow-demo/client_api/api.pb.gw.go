...
var (
	filter_Ping_GetDummyData_0 = &utilities.DoubleArray{Encoding: map[string]int{}, Base: []int(nil), Check: []int(nil)}
)

func request_Ping_GetDummyData_0(ctx context.Context, marshaler runtime.Marshaler, client PingClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	var protoReq DummyDataRequest
	var metadata runtime.ServerMetadata

	if err := runtime.PopulateQueryParameters(&protoReq, req.URL.Query(), filter_Ping_GetDummyData_0); err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "%v", err)
	}

	msg, err := client.GetDummyData(ctx, &protoReq, grpc.Header(&metadata.HeaderMD), grpc.Trailer(&metadata.TrailerMD))
	return msg, metadata, err
...
func RegisterPingHandlerClient(ctx context.Context, mux *runtime.ServeMux, client PingClient) error {
	...
	mux.Handle("GET", pattern_Ping_GetDummyData_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		ctx, cancel := context.WithCancel(req.Context())
		defer cancel()
		if cn, ok := w.(http.CloseNotifier); ok {
			go func(done <-chan struct{}, closed <-chan bool) {
		...
		resp, md, err := request_Ping_GetDummyData_0(rctx, inboundMarshaler, client, req, pathParams)
		ctx = runtime.NewServerMetadataContext(ctx, md)
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
		...
		forward_Ping_GetDummyData_0(ctx, mux, outboundMarshaler, w, req, resp, mux.GetForwardResponseOptions()...)
...
var (
	...
	pattern_Ping_GetDummyData_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0}, []string{"getDummyData"}, ""))
)

var (
	forward_Ping_SayHello_0 = runtime.ForwardResponseMessage

	forward_Ping_GetDummyData_0 = runtime.ForwardResponseMessage
)
